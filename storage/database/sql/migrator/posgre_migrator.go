...
func NewPostgreMigrator(db *sql.DB) *postgremigrator {
...
func (migrator *postgremigrator) Migrate() error {
	sourceInstance, err := httpfs.New(http.FS(PostgreSQLMigrations), "posgresql_migrations")
	if err != nil {
		return fmt.Errorf("invalid source instance, %w", err)
	}
	targetInstance, err := postgres.WithInstance(migrator.db, new(postgres.Config))
	if err != nil {
		return fmt.Errorf("invalid target postgre instance, %w", err)
	}
